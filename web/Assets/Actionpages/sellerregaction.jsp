...
//checking if request cotains multipart data
            ...
            //f_item=(FileItem)itr.next();
                        ...
                        if (field_name.equals("photo"))//write the name of image here it is img_product
                        {
                            ...
                            String file_path = proj_path.getParentFile().getParentFile().getPath() + "\\Web\\Assets\\userphoto\\";//path for storing image
                            Random r = new Random();//to generate random number
                            int r_num = r.nextInt(1111) + 999;//values between this are generated

                            photo = "sellerphoto_" + r_num + ext;//generating random number for images so this name and format of imagewil be stored inside the server
                        ...
                        if (field_name.equals("proof"))//write the name of image here it is img_product
                        {
                            ...
                            //setting path to store image
                            File proj_path = new File(config.getServletContext().getRealPath("/"));
                            String file_path = proj_path.getParentFile().getParentFile().getPath() + "\\Web\\Assets\\userproof\\";//path for storing image
                            Random r = new Random();//to generate random number
                            int r_num = r.nextInt(1111) + 999;//values between this are generated

                            proof = "sellerproof_" + r_num + ext;//generating random number for images so this name and format of imagewil be stored inside the server
                            //creating a file object
           ...
           //to upload proof of doctor
                ...
                String InsQry = "insert into tbl_seller(seller_name,seller_contact,seller_email,seller_address,seller_photo,seller_proof,place_id,seller_password,seller_doj)values('" + value[0] + "','" + value[1] + "','" + value[2] + "','" + value[3] + "','" + photo + "','" + proof + "','" + value[4] + "','" + value[5]+ "',curdate())";
   ...
   //System.out.println(InsQry);
        ...
        <script type="text/javascript">
            ...
            setTimeout(function() {
                window.location.href = '../../Guest/Sellerreg.jsp';
