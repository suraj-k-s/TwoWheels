...
//checking if request cotains multipart data
            ...
            //f_item=(FileItem)itr.next();
                        ...
                        if (field_name.equals("photo"))//write the name of image here it is img_product
                        {
                            ...
                            String file_path = proj_path.getParentFile().getParentFile().getPath() + "\\Web\\Assets\\usedveh\\";//path for storing image
                        ...
                        if (field_name.equals("rc"))//write the name of image here it is img_product
                        {
                            String ext = file_name.substring(file_name.lastIndexOf("."));//means the extension is stored to variable ext
                            //setting path to store image
                            File proj_path = new File(config.getServletContext().getRealPath("/"));
                            ...
                            rc = "rc_" + r_num + ext;//generating random number for images so this name and format of imagewil be stored inside the server
                            //creating a file object
                            savedFile = new File(file_path + rc);//fn contains name of file
                            try {
                                //writing the file object
                                f_item.write(savedFile);  //writing to server means the folder             
           ...
           //to upload proof of doctor
              ...
              //String str1="insert into tbl_user ()";//inserting file
                String InsQry = "insert into tbl_usedvehicle(vehicle_name,type_id,displacement_id,brand_id,vehicle_photo,vehicle_rc,vehicle_price,vehicle_year,vehicle_audometer,seller_id,vehicle_date)values('" + value[0] + "','" + value[1] + "','" + value[2] + "','" + value[3] + "','" + photo + "','" + rc + "','" + value[4] + "','" + value[5] + "','" + value[6]+ "','"+session.getAttribute("sellerid")+"',curdate())";
   ...
   System.out.println(InsQry);
        ...
        <script type="text/javascript">
            ...
            setTimeout(function() {
                window.location.href = '../../Seller/ausedveh.jsp';
